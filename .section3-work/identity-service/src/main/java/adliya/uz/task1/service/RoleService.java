...
public class RoleService {
    ...
    public String assignRoleToUser(AssignRoleRequest request, String actorEmail) {
        ...
        if (!previousRole.equals(role.getName())) {
            user.setRole(role);
    ...
    public RoleResponse assignPermissions(Long roleId, Set<Long> permissionIds) {
        ...
        if (existingPermissionIds.equals(permissionIds)) {
            return RoleResponse.from(role);
        }

        role.setPermissions(permissions);
        RoleResponse response = RoleResponse.from(roleRepository.save(role));
        List<Long> affectedUserIds = userRepository.findAllByRole_Name(role.getName()).stream()
                .map(User::getId)
                .toList();
        affectedUserIds.forEach(sessionRevocationService::invalidateAllSessions);
        return response;
    }
